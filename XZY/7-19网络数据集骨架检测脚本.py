...
from XZY.deep_sort import *

import cv2

from mmpose.apis import (inference_top_down_pose_model, init_pose_model,
                         vis_pose_result, vis_pose_tracking_result)
from mmdet.apis import show_result_pyplot
from XZY.libs.config import coco_idx_keypoint_map
from XZY.libs.save_csv_lib import save_csv_for_per_video, show_keypoint, save_csv_for_per_video_tracking
...
det_model = None
pose_model = None
deepsort = DeepSort("./XZY/deep_sort/deep/checkpoint/ckpt.t7")
...
def main(args, is2021_data=False, USE_TRACK=True):
    """Visualize the demo images.
    Using mmdet to detect the human.
    is2021_data: True: 针对2021采集的数据做了特定的噪声处理
    allow_multi_person: 允许检测多人，但此时保存的csv只会保留第一个人的
    """
    global det_model, pose_model, deepsort

    deepsort.tracker.clear()  # 下一个视频清空上一个视频的轨迹
    ...
    while (cap.isOpened()):
        ...
        if USE_TRACK:
            "--------------------目标跟踪---------------------------"
            bbox_xyxy = mmdet_results[0][:, :4]
            cls_conf = mmdet_results[0][:, 4:]
            bbox_xcycwh = xyxy2xywh(bbox_xyxy)
            outputs = deepsort.update(bbox_xcycwh, cls_conf, img)  # [x,y,x,y,id]

            mmtracking_results = {}
            mmtracking_results['track_results'] = []
            if len(outputs) > 0:
                outputs = outputs[:, [4, 0, 1, 2, 3]]
                mmtracking_results['track_results'].append(np.hstack([outputs, np.ones((outputs.shape[0], 1))]))
            else:
                mmtracking_results['track_results'].append([])
        ...
        if len(person_results) > 0 and is2021_data:
        ...
        if len(pose_results) > 0:
            if USE_TRACK:
                save_csv_for_per_video_tracking(pose_results)
            else:
...
if __name__ == '__main__':
    ...
    load_models(args)
    video_dir_list = [
        # "/home/xzy/Data/扶梯项目数据集/其他数据集/行为识别数据集/网络部分行为识别视频/蹲下起立",
        # "/home/xzy/Data/扶梯项目数据集/其他数据集/行为识别数据集/网络部分行为识别视频/行走",
        # "/home/xzy/Data/扶梯项目数据集/其他数据集/行为识别数据集/网络部分行为识别视频/举手放下",
    ...
    for video_dir in video_dir_list:
        for video_dir2 in os.listdir(video_dir):
            ...
            for video_name in os.listdir(os.path.join(video_dir, video_dir2)):
                ...
                #     continue
                video_path = os.path.join(video_dir, video_dir2, video_name)
                print(f'deal with video : {video_path}')
                args.video_path = video_path  # 设置检测视频路径
                save_path = os.path.join("vis_results", os.path.basename(video_dir), video_dir2)
                args.out_video_root = save_path  # 设置图片保存路径
                gol.set_value("video_path", value=os.path.join(save_path, video_name))
