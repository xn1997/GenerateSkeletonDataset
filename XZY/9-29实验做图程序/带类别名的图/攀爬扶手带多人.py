...
def main(args, is2021_data=False):
    ...
    for person_id in range(len(pose_results_back)):
        ...
        if person_id in [3, 4, 5]:
            continue
        pose_results.append(person_info)

        # action_name = "   " + str(person_id) # 标注每个人的ID
        # cv2.putText(img, action_name, tuple(person_info['keypoints'][0][0:2]), cv2.FONT_HERSHEY_SIMPLEX, fontScale=0.8, color=(255, 0, 0), thickness=2)
    ...
    for person_id in range(len(pose_results)):
        ...
        if person_id in [1] or len(pose_results) == 1:
            action_name = "CH"
...
python XZY/9-29实验做图程序/带类别名的图/攀爬扶手带多人.py \
...
if __name__ == '__main__':
    ...
    image_dir = "/media/xzy/My Passport/实验视频/单人行为类别/CH攀爬"
